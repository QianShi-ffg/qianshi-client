    ...
    <el-container>
      ...
      <el-container>
        <el-header height="70px"
          ><nav class="toolbar">
            <span @click="minimize"
              ><img src="../assets/icon/zuixiaohua.svg" alt=""
            /></span>
            <span @click="maximize"
              ><img src="../assets/icon/zuidahua.svg" alt=""
            /></span>
            <span @click="close" class="close"
              ><img
                src="../assets/icon/guanbi.svg"
                alt=""
                @click="close"
            /></span>
          </nav>
...
const currentListItem = ref<string>('')
...
const init = async() => {
  ...
  chatList.value = res.filter((item:any) => {
    return item.id !== sessionStorage.getItem("uuid");
  });
}
init()

const selectChat = (item:any, i:any) => {
  const dom:any = document.getElementsByClassName("list")
  let list = [...dom];
  list.forEach((item) => {
    item.className = "list";
  });
  let crrList:any = document.getElementById(`list${i}`);
  crrList.classList.add("is_active");
  router.push({ name: "chatFrame", params: { id: item.id } });
  toName.value = item.name;
  store.setCleanMessage(true);
  // 再次请求当前数据
  //-----------待开发
  let aaa = setTimeout(() => {
    store.setCleanMessage(false);
    clearTimeout(aaa);
  }, 1000);
  //
}

const clickMenu = (item:any, i:any) => {
  // currentListItem
  console.log(item, i);
}
