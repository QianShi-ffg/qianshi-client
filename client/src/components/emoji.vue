<template>
  <div id="emojiDialog">
    <el-dialog title="" v-model="emoji" width="360px" :before-close="close" :modal="false" :show-close="false"
     class="emojiDialog" :append-to-body="false" top="22vh">
    <el-card class="box-card">
      <div class="wwxface" :class="`wwxface${i}`" style="width: 38px; height: 38px" v-for="(item, i) in emojiList"
        :key="i" :title="item" @click="emojiClick(item, i)"></div>
      <div class="bar"></div>
    </el-card>
  </el-dialog>
  </div>
...
onMounted(() => {
  ...
  let valueArr: any = [];
  let KeyArr: any = [];
  emojiMap.forEach((value: any, key: any) => {
    valueArr.push(value);
    KeyArr.push(key);
  });
